...
const DetailTransactionScreen = React.memo(
  ({route}: {route: RouteProp<RootStackParams, 'DetailTransactionScreen'>}) => {
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.bodyContainer}>
          <View style={styles.bankContainer}>
            <Text style={[styles.textStyle, {fontSize: widthResponsive(20)}]}>
          ...
          <View style={styles.doubleTextContainer}>
            <DoubleText
              title={item.beneficiary_name.toString().toUpperCase()}
              value={item.account_number.toString()}
              style={{flex: 3}}
            />
            <DoubleText
              title={'NOMINAL'}
              value={formatCurrency(parseInt(item.amount.toString()))}
              style={{flex: 2}}
            />
          </View>
          <Gap height={20} />
          <View style={styles.doubleTextContainer}>
            ...
            <DoubleText
              title={'KODE UNIK'}
              value={item.unique_code.toString()}
              style={{flex: 2}}
            />
          </View>
          <Gap height={20} />
          <View style={styles.doubleTextContainer}>
            <DoubleText
              title={'WAKTU DIBUAT'}
              value={formatDate(item.created_at.toString())}
            />
          </View>
        </View>
      </SafeAreaView>
